...
| `bash`    | `command`, `timeout`                                                                      | Run shell commands in your workspace via an in-process interpreter (host-bash fallback), with a timeout and instant cancellation; detach requests (`&`, `nohup`) are refused — use the `process` tool |
