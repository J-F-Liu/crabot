...
pub(crate) fn open_settings(app: &mut App) -> Task<Message> {
    ...
    app.settings_dialog.appearance = app.settings.appearance;
    app.settings_dialog.font_scale = app.settings.font_scale;
    app.settings_dialog.iced_backend = app.settings.iced_backend.clone();
...
pub(crate) fn handle_event(app: &mut App, event: SettingsEvent) -> Task<Message> {
    match event {
        ...
        SettingsEvent::SetAppearance(appearance) => {
            ...
            app.apply_appearance(appearance);
        }
        SettingsEvent::SetIcedBackend(_) => {
            app.settings_dialog.update(event);
            app.settings.iced_backend = app.settings_dialog.iced_backend.clone();
        }
        SettingsEvent::SetFontScale(_) => {
            app.settings_dialog.update(event);
            app.set_font_scale(app.settings_dialog.font_scale);
        }
